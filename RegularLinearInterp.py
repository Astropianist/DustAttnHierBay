...
def regular_grid_interp(points, values, coords):
    ...
    """
    points = [tt.as_tensor_variable(p) for p in points]
    ndim = len(points)
    values = tt.as_tensor_variable(values)
    coords = tt.as_tensor_variable(coords)
...
def regular_grid_interp_scipy(points, values, coords):
    rgi = RGIScipy(points,values,bounds_error=False)
    return rgi(coords)
